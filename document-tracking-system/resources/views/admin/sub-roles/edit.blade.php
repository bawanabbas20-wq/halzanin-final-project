...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-4 animate-fade-in">
        ...
        <div>
            <h2 class="text-2xl font-bold text-brand dark:text-white font-outfit">Edit Sub-Role</h2>
            <p class="text-sm text-gray-500 dark:text-gray-400">Editing: <strong>{{ $subRole->name }}</strong></p>
        </div>
    </div>
    ...
    <form method="POST" action="{{ route('admin.sub-roles.update', $subRole->id) }}" class="space-y-6">
        @csrf @method('PATCH')
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <textarea name="description" rows="2"
                              class="block w-full rounded-[10px] border-gray-200 dark:border-gray-700 dark:bg-[#0f172a] dark:text-white focus:border-brand focus:ring-0 text-sm px-4 py-3 transition-colors resize-none">{{ old('description', $subRole->description) }}</textarea>
                </div>
            </div>
        </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                @foreach($allPermissions as $perm)
                    @php
                        $meta    = $permMeta[$perm];
                        $icon    = $permIcons[$perm];
                        $active  = in_array($perm, old('permissions', $grantedPerms));
                    @endphp
                    <label class="permission-card relative flex items-start gap-4 p-4 rounded-[12px] border-2 cursor-pointer transition-all select-none
                                  {{ $active ? 'border-brand bg-brand/5 dark:bg-indigo-900/20' : 'border-gray-200 dark:border-gray-700 bg-white dark:bg-[#0f172a] hover:border-gray-300 dark:hover:border-gray-600' }}"
                           data-perm="{{ $perm }}">
                        <input type="checkbox" name="permissions[]" value="{{ $perm }}"
                               class="sr-only perm-checkbox" {{ $active ? 'checked' : '' }}>

                        <div class="shrink-0 w-10 h-10 rounded-[10px] flex items-center justify-center transition-colors
                                    {{ $active ? 'bg-brand text-white' : 'bg-gray-100 dark:bg-slate-800 text-gray-500 dark:text-gray-400' }}">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">{!! $icon !!}</svg>
                        </div>

                        <div class="flex-1 min-w-0">
                            <p class="text-sm font-bold {{ $active ? 'text-brand dark:text-indigo-400' : 'text-gray-900 dark:text-white' }}">{{ $meta['label'] }}</p>
