...
<div class="space-y-6 lg:space-y-8">
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-2xl font-bold text-brand dark:text-white font-outfit">Document Vault</h2>
            <p class="text-gray-500 dark:text-gray-400 text-sm mt-1">Securely store your official documents.</p>
        </div>
                ...
                <div class="bg-white dark:bg-[#1e293b] rounded-[16px] p-5 shadow-sm border border-gray-100 dark:border-gray-800 flex flex-col justify-between">
                    <div>
                        <div class="flex items-center justify-between mb-3">
                            ...
                            <span class="text-xs font-semibold {{ $expiresColor }}">Expires in {{ max(0, $daysLeft) }}d</span>
                        </div>
                        <h3 class="font-bold text-gray-900 dark:text-white">{{ $doc->original_name }}</h3>
                        <p class="text-xs text-gray-500 mt-1">Scanned: {{ $doc->created_at->format('M d, Y') }}</p>
                    </div>
