<x-halzanin-auth-layout>
    ...
    <div class="w-full">
        <h2 class="text-[22px] font-semibold text-brand dark:text-white mb-6 font-outfit">Welcome back</h2>
        ...
        <form method="POST" action="{{ route('login') }}" class="space-y-4">
            ...
            <div class="flex items-center justify-between pt-1">
                <label for="remember_me" class="inline-flex items-center">
                    ...
                    <span class="ltr:ml-2 rtl:mr-2 text-sm text-gray-600 dark:text-gray-400">{{ __('Remember me') }}</span>
                </label>
                @if (Route::has('password.request'))
                    <a class="text-sm font-medium text-brand dark:text-indigo-400 hover:underline" href="{{ route('password.request') }}">
                        Forgot password?
                    </a>
                @endif
            </div>
            ...
            <!-- Login Button -->
            <button type="submit" class="w-full h-[52px] bg-brand text-white rounded-[10px] font-semibold font-outfit shadow-brand-btn hover:shadow-brand-btn-hover hover:-translate-y-[1px] transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-brand dark:focus:ring-offset-[#1e293b] mt-4">
                Log in
            </button>
            ...
            <div class="text-center text-sm text-gray-600 dark:text-gray-400">
                ...
                <a href="{{ route('register') }}" class="font-semibold text-brand dark:text-indigo-400 hover:underline">Register</a>
            </div>
        </form>
    </div>
