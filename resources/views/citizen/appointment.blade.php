...
<div class="w-full max-w-[580px] mx-auto">
    <div class="bg-white dark:bg-[#1e293b] rounded-[16px] p-6 lg:p-8 shadow-[0_10px_15px_rgba(0,0,0,0.08)] relative overflow-hidden">
        ...
        <div class="relative flex justify-between mb-10" id="step-indicator">
            ...
            @foreach([1=>'Personal Info', 2=>'Appointment', 3=>'Documents', 4=>'Review'] as $n => $label)
            <div class="flex flex-col items-center w-1/4">
                ...
                <span class="text-xs font-semibold mt-2 text-center {{ $n === 1 ? 'text-brand dark:text-indigo-400' : 'text-gray-400' }}">{{ $label }}</span>
            </div>
            @endforeach
        </div>
            ...
            <form method="POST" action="{{ route('citizen.appointment.store') }}" id="appointment-form"
                ...
                <div class="w-1/4 shrink-0 px-1" id="step-1">
                    <div class="space-y-4">
                        ...
                        <div class="relative">
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">National ID Number</label>
                            <div class="absolute inset-y-0 left-0 mt-6 flex items-center pl-4 pointer-events-none">
                        ...
                        <div class="pt-6">
                            <button type="button" onclick="nextStep(2)" class="w-full h-[52px] bg-brand text-white rounded-[10px] font-semibold shadow-brand-btn hover:shadow-brand-btn-hover hover:-translate-y-[1px] transition-all">Continue</button>
                        </div>
                    </div>
                </div>
                ...
                <div class="w-1/4 shrink-0 px-1" id="step-2">
                    <div class="space-y-4">
                        ...
                        <div class="relative">
                            ...
                            <select id="document_type" name="document_type" required class="block w-full h-[48px] pl-11 pr-4 rounded-[10px] border-gray-300 dark:border-gray-600 dark:bg-[#0f172a] dark:text-white focus:border-brand focus:ring-0 transition-all">
                                ...
                                @foreach(['Passport Renewal','New Passport','ID Card','Birth Certificate','Other'] as $dt)
                                    <option value="{{ $dt }}" {{ old('document_type', session('appointment_draft.document_type')) == $dt ? 'selected' : '' }}>{{ $dt }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="relative">
                        ...
                        <div class="pt-4 flex space-x-3">
                            <button type="button" onclick="nextStep(1)" class="w-1/3 h-[52px] bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 rounded-[10px] font-semibold transition-all">Back</button>
                            <button type="button" onclick="nextStep(3)" class="w-2/3 h-[52px] bg-brand text-white rounded-[10px] font-semibold shadow-brand-btn hover:shadow-brand-btn-hover hover:-translate-y-[1px] transition-all">Continue</button>
                        </div>
                    </div>
                </div>
                ...
                <div class="w-1/4 shrink-0 px-1" id="step-3">
                    <div class="mb-4 p-4 bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-100 dark:border-indigo-800 rounded-[10px]">
                        <p class="text-sm font-semibold text-brand dark:text-indigo-300 mb-1">Required Documents</p>
                        <p id="doc-type-label" class="text-xs text-indigo-600 dark:text-indigo-400">Select a document type in Step 2 first.</p>
                    </div>
                    <div id="doc-upload-fields" class="space-y-4">
                        {{-- Dynamically rendered by JS --}}
                    </div>
                    <p id="upload-hint" class="text-xs text-amber-600 dark:text-amber-400 mt-3 hidden">⚠ Please upload all required documents to continue.</p>
                    <div class="pt-6 flex space-x-3">
                        <button type="button" onclick="nextStep(2)" class="w-1/3 h-[52px] bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 rounded-[10px] font-semibold transition-all">Back</button>
                        <button type="button" onclick="nextStep(4)" id="btn-to-review" class="w-2/3 h-[52px] bg-brand text-white rounded-[10px] font-semibold shadow-brand-btn hover:shadow-brand-btn-hover hover:-translate-y-[1px] transition-all">Continue</button>
                    </div>
                </div>
                ...
                <div class="w-1/4 shrink-0 px-1" id="step-4">
                    <div class="bg-gray-50 dark:bg-slate-800/50 rounded-[10px] border border-gray-200 dark:border-gray-700 p-5 mb-4">
                        ...
                        <div class="grid grid-cols-2 gap-y-4 gap-x-2">
                            <div><p class="text-[11px] text-gray-500 uppercase tracking-wide">Full Name</p><p id="sum-name" class="font-medium text-gray-900 dark:text-white text-sm mt-0.5 truncate"></p></div>
                            <div><p class="text-[11px] text-gray-500 uppercase tracking-wide">National ID</p><p id="sum-id" class="font-medium text-gray-900 dark:text-white text-sm mt-0.5 truncate"></p></div>
                            <div><p class="text-[11px] text-gray-500 uppercase tracking-wide">Document Type</p><p id="sum-doc" class="font-medium text-gray-900 dark:text-white text-sm mt-0.5 truncate"></p></div>
                            <div><p class="text-[11px] text-gray-500 uppercase tracking-wide">Appointment</p><p id="sum-date" class="font-medium text-gray-900 dark:text-white text-sm mt-0.5 truncate"></p></div>
                        </div>
                    </div>
                    <div id="sum-files-section" class="bg-gray-50 dark:bg-slate-800/50 rounded-[10px] border border-gray-200 dark:border-gray-700 p-5 mb-4">
                        <h3 class="text-sm font-semibold text-brand dark:text-indigo-400 mb-3 uppercase tracking-wider">Uploaded Documents</h3>
                    ...
                    <div class="flex space-x-3">
                        <button type="button" onclick="nextStep(3)" class="w-1/3 h-[52px] bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 rounded-[10px] font-semibold transition-all">Back</button>
                        <button type="submit" class="w-2/3 h-[52px] bg-accent text-white rounded-[10px] font-semibold shadow-brand-btn hover:shadow-brand-btn-hover hover:-translate-y-[1px] transition-all">Submit Appointment</button>
                    </div>
                </div>
    ...
    <x-modal name="confirm-appointment">
        <div class="p-6">
            ...
            <p class="text-sm text-gray-600 dark:text-gray-400 mb-5">Please confirm your appointment details are correct. All uploaded documents and details will be sent for review.</p>
            ...
            <div class="flex items-center justify-end gap-3">
                <button type="button" x-on:click="$dispatch('close-modal','confirm-appointment')" class="px-5 py-2.5 bg-gray-100 hover:bg-gray-200 dark:bg-slate-700 dark:hover:bg-slate-600 text-gray-700 dark:text-gray-200 rounded-[10px] font-semibold transition-colors text-sm">Go Back</button>
                <button type="button" x-on:click="document.getElementById('appointment-form').__x.$data.confirmed = true; document.getElementById('appointment-form').submit()" class="px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-[10px] font-semibold shadow-md transition-colors text-sm">Submit Now</button>
            </div>
        </div>
    </x-modal>
</div>
