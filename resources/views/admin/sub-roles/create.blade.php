...
<div class="max-w-2xl mx-auto pb-10">
    ...
    <div class="flex items-center justify-between mb-6 animate-fade-in">
        <div>
            <h2 class="text-2xl font-bold font-outfit text-gradient">Create Sub-Role</h2>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Define a new permission set for staff members</p>
        </div>
    ...
    <div class="bg-white dark:bg-[#1F1F1F] rounded-2xl shadow-sm border border-gray-100 dark:border-slate-800 overflow-hidden animate-fade-up" style="animation-delay:100ms">
        ...
        <form method="POST" action="{{ route('admin.sub-roles.store') }}" class="p-6 space-y-6">
            ...
            <div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    @foreach($permissionsMap as $perm => $meta)
                        @php $checked = in_array($perm, old('permissions', [])); @endphp
            ...
            <div class="flex justify-end gap-3 pt-2 border-t border-gray-100 dark:border-slate-800">
                ...
                <button type="submit"
                        class="px-6 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-semibold rounded-xl shadow-sm transition-all hover:-translate-y-px">
                    Create Sub-Role
                </button>
            </div>
        </form>
...
<script>
    // Live toggle visual state for checkboxes
