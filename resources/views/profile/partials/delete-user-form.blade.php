    ...
    <x-modal name="confirm-user-deletion" :show="$errors->userDeletion->isNotEmpty()" focusable>
        <form method="post" action="{{ route('profile.destroy') }}" class="p-6 bg-white dark:bg-[#1e293b]">
            ...
            <div class="mt-6 flex justify-end space-x-3 rtl:space-x-reverse">
                <button type="button" x-on:click="$dispatch('close')" class="px-4 py-2 text-sm font-semibold text-gray-600 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white transition-colors">
                    {{ __('Cancel') }}
                </button>

                <button type="submit" class="px-6 h-[42px] bg-red-600 text-white rounded-[10px] font-semibold font-outfit shadow-sm hover:bg-red-700 transition-all">
                    {{ __('Delete Account') }}
                </button>
            </div>
        </form>
