<section class="space-y-6">
    <header>
        ...
        <p class="mt-1 text-sm text-red-500/80 dark:text-red-400/80" data-i18n="profile.delete_desc">
    ...
    <button
        ...
        data-i18n="profile.delete_account"
    ...
    <x-modal name="confirm-user-deletion" :show="$errors->userDeletion->isNotEmpty()" focusable>
        <form method="post" action="{{ route('profile.destroy') }}" class="p-6 bg-white dark:bg-[#1F1F1F]">
            ...
            <h2 class="text-lg font-bold text-gray-900 dark:text-white" data-i18n="profile.delete_confirm_title">
            ...
            <div class="mt-6">
                <label for="password" class="sr-only" data-i18n="auth.password">{{ __('Password') }}</label>
                <input
                    id="password"
                    name="password"
                    type="password"
            ...
            <div class="mt-6 flex justify-end space-x-3 rtl:space-x-reverse">
                <button type="button" x-on:click="$dispatch('close')" class="px-4 py-2 text-sm font-semibold text-gray-600 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white transition-colors" data-i18n="common.cancel">
                    {{ __('Cancel') }}
                </button>

                <button type="submit" class="px-6 h-[42px] bg-red-600 text-white rounded-[10px] font-semibold font-outfit shadow-sm hover:bg-red-700 transition-all" data-i18n="profile.delete_account">
                    {{ __('Delete Account') }}
                </button>
            </div>
        </form>
